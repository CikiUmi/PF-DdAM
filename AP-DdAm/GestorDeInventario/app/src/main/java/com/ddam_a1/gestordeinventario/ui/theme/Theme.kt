...
//  COLORES EXTRA
//
//  `MaterialTheme.colorScheme` es una lista CERRADA. `lightColorScheme()` y
//  `darkColorScheme()` reciben exactamente los roles que Material define, y
//  `correct` no es uno de ellos: por eso los 24 errores (6 esquemas x 4 papeles).
//
//  El color extra no se borra, se muda. Va en un paquete propio que viaja al
//  lado del tema, con el mismo mecanismo que usa Material por dentro:
...
data class ColoresExtra(
    /** Confirmado / correcto / existencias sanas. El verde olivo #506528. */
    val correct: ColorFamily
)

private val coloresExtraClaro = ColoresExtra(
    correct = ColorFamily(correctLight, onCorrectLight, correctContainerLight, onCorrectContainerLight)
)

private val coloresExtraOscuro = ColoresExtra(
    correct = ColorFamily(correctDark, onCorrectDark, correctContainerDark, onCorrectContainerDark)
)
 ...
 * alguien dibuja un componente FUERA de GestorDeInventarioTheme (pasa en los
 * previews a medio hacer), es mejor que se vea con el color mal a que se vea
 * invisible.
 */
...
fun GestorDeInventarioTheme(
    ...
    // Dynamic color (Material You) toma los colores del fondo de pantalla del
    // usuario y PISA tu paleta. En false para que mande tu tema.

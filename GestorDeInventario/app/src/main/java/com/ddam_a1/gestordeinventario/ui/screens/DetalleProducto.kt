...
fun PantallaDetalleProducto(
    ...
    if (producto == null) {
        Marco(barra = { BarraSuperior("Producto", onAtras = onAtras) }) {
            item {
                Text("Este producto ya no existe.",
    ...
    Marco(barra = {
            ...
            TarjetaSuave {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    if (producto.esBajoPedido) {
                        ...
                        Insignia(producto.stockDisponible.toString() + " piezas en stock",
        ...
        if (receta.isEmpty()) {
            ...
            items(receta.size) { i ->
                ...
                FilaLista(
                    ...
                    subtitulo = cant(renglon.cantidadUsada) + " " + renglon.unidad +
                        " x " + dinero(renglon.costoUnitario),
                    valor = dinero(renglon.costoUnitario * renglon.cantidadUsada)
                )
            }
        }
        ...
        item {
            ...
            if (!producto.esBajoPedido) {
                BotonPrincipal("Registrar produccion") { onProducir() }
            } else {
